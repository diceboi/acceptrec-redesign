...
export function Navbar() {
    ...
    return (
        <header 
            className={`fixed top-0 z-50 flex w-full items-center justify-between px-8 md:px-16 transition-all duration-300
                ${isSolid ? 'bg-white shadow-sm h-16 backdrop-filter-inverse(100%) backdrop-blur-sm' : 'bg-transparent h-25'}`}
        ...
        >
            ...
            <div className="flex items-center">
                <Link href="/">
                    <Image 
                        src={isSolid ? "/Accept-Stacked-Logo-with-Strapline-RGB300.webp" : "/acceptrec-white-logo.webp"} 
                        alt="AcceptRec Logo" 
                        width={200} 
                        height={60} 
            ...
            <nav className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-8 h-full">
                {Object.values(dropdownData).map((menu) => (
                    ...
                    >
                        <Link href="#" className={`font-sans text-[15px] font-semibold transition-colors
                            ${activeDropdown === menu.id ? 'text-green-6' : 
                ...
                {/* Contact (No Dropdown) */}
                <div className="h-full flex items-center" onMouseEnter={() => {
                        if (timeoutRef.current) clearTimeout(timeoutRef.current);
                        setActiveDropdown(null);
                    }}>
                    <Link href="#" className={`font-sans text-[15px] font-semibold transition-colors
                ...
                <AnimatePresence>
                    {activeDropdown && dropdownData[activeDropdown] && (
                        <motion.div
                            ...
                            exit={{ opacity: 0, y: 10, transition: { duration: 0.2, delay: 0.1 } }}
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col gap-3">
                                            {dropdownData[activeDropdown].items.map((item, idx) => (
                                                ...
                                                >
                                                    <span className="font-medium text-gray-900 group-hover:text-green-6">{item.label}</span>
                                                    <span className="text-gray-300 transition-transform group-hover:translate-x-1 group-hover:text-green-5">→</span>
                                                </Link>
                                            ))}
                                        </div>
                                    </motion.div>
            ...
            <div className="flex items-center md:hidden">
                <button className={`p-2 transition-colors focus:outline-none hover:text-green-5
