    ...
    <section className="relative flex min-h-[80vh] w-full items-center justify-center overflow-hidden bg-navy-900 pt-32 pb-20">
      ...
      <div className="relative z-10 mx-auto max-w-[1140px] px-6 text-center">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8 inline-flex items-center gap-2 rounded-full border border-teal-5/30 bg-teal-5/10 px-4 py-2">
          ...
          <span className="text-sm font-semibold text-teal-4">Cut No-Shows 75%</span>
          <span className="h-4 w-[1px] bg-white/10 mx-1" />
          <span className="text-[10px] text-[#8B98AB] uppercase tracking-widest font-semibold">Case Study →</span>
        </motion.div>
        <motion.h1 className="font-sans text-5xl font-semibold leading-[1.05] tracking-tight text-white md:text-6xl lg:text-7xl mb-8" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
          What If Your Agency<br /><span className="text-teal-5 italic">Actually Delivered?</span>
        </motion.h1>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto pt-12 border-t border-white/10">
          {[{ v: "55 Workers", l: "In 3 Days For InPost" }, { v: "17 Days", l: "To Preferred Partner" }, { v: "190+", l: "Companies Trust Us" }].map((s, i) => (
            <div key={i} className="text-center"><div className="text-teal-4 font-semibold text-2xl">{s.v}</div><div className="text-[#8B98AB]/70 text-[10px] uppercase tracking-widest font-semibold">{s.l}</div></div>
          ))}
        </motion.div>
      </div>
             ...
             <div className="relative z-10 mx-auto max-w-[1140px] px-6">
                <div className="grid grid-cols-1 lg:grid-cols-[1fr,1.3fr] gap-20">
                    ...
                    <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
                        <span className="mb-4 inline-block text-[11px] font-bold uppercase tracking-[0.2em] text-[#00A99D]">WE DO THINGS DIFFERENTLY</span>
                        <h2 className="text-4xl font-semibold text-white mb-12 tracking-tight">The Accept Difference</h2>
                        <div className="space-y-12">
                             <div className="glass-card rounded-2xl p-8 border-red-500/10 bg-red-500/5">
                                 ...
                                 <ul className="space-y-4">
                                     {["Send whoever's available", "Disappear when there's a problem", "Different contact every month", "\"We'll try our best\" fill rates"].map((text, i) => (
                                         <li key={i} className="text-[#8B98AB] text-sm font-semibold flex items-center gap-3">
                             ...
                             <div className="glass-card rounded-2xl p-8 border-teal-5/20 bg-teal-5/5 relative overflow-hidden">
                                 ...
                                 <ul className="relative space-y-4">
                                     {["Every worker briefed on your site", "No-show? Free replacement", "Same team. Real relationships.", "98% fill rate. We deliver."].map((text, i) => (
                                         <li key={i} className="text-white font-semibold flex items-center gap-3">
                    ...
                    {/* Right Side: Form */}
                    <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="glass-card rounded-2xl p-10 md:p-16 relative">
                         <div className="mb-12">
                             <h3 className="text-3xl font-semibold text-white mb-4 tracking-tight leading-tight">Let&apos;s fix your staffing.</h3>
            ...
            <div className="pointer-events-none absolute -right-64 top-1/2 h-[600px] w-[600px] -translate-y-1/2 rounded-full bg-teal-5/10 blur-[130px]" />
             <div className="relative z-10 mx-auto max-w-[1140px] px-6">
                <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
                     <span className="mb-4 inline-block text-[11px] font-bold uppercase tracking-[0.2em] text-[#00A99D]">WHAT HAPPENS NEXT</span>
                     <h2 className="text-4xl font-semibold text-white tracking-tight mb-8">From First Call to Full Shift.</h2>
                </motion.div>
                    ...
                    {steps.map((s, i) => (
                        <div key={i} className="relative z-10 text-center flex flex-col items-center group">
                             <div className="w-24 h-24 rounded-full glass-card flex items-center justify-center text-teal-4 mb-8 shadow-2xl group-hover:border-teal-5/50 transition-all relative">
                                 <div className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-teal-5 text-black font-semibold text-xs flex items-center justify-center">0{i+1}</div>
                                 <s.i size={40} />
                             </div>
                             <h4 className="text-white font-semibold text-xl mb-4 leading-tight">{s.t}</h4>
                             <p className="text-[#8B98AB] text-sm leading-relaxed max-w-xs">{s.d}</p>
                        </div>
                    ))}
                </div>
            </div>
    ...
    <section className="py-24 bg-navy-900">
      <div className="mx-auto max-w-[1140px] px-6">
        <div className="text-center mb-16">
          <h3 className="text-white text-3xl font-semibold tracking-tight mb-4">Trusted by 190+ Companies</h3>
          <p className="text-[#8B98AB] text-sm font-medium uppercase tracking-widest">Want to join them?</p>
          <div className="mt-12 flex flex-wrap justify-center gap-4">
            {clients.map((c) => (
              <div key={c.name} className="bg-white rounded-2xl p-6 flex items-center justify-center w-40 h-24 hover:scale-[1.03] transition-all duration-300 shadow-xl shadow-black/10">
        ...
        <div className="text-center">
          <h3 className="text-[#8B98AB]/70 text-[11px] font-semibold uppercase tracking-[0.2em] mb-12">Fully Licensed & Compliant</h3>
          <div className="flex flex-wrap justify-center items-center gap-6">
            {accreditations.map((a) => (
              <div key={a.name} className="bg-white rounded-2xl p-6 flex items-center justify-center w-56 h-24 hover:scale-[1.03] transition-all duration-300 shadow-xl shadow-black/10">
                <img src={a.src} alt={a.name} className="h-12 max-w-full object-contain transition-all duration-500" title={a.name} />
              </div>
            ))}
          </div>
        </div>
