    ...
    <section className="relative flex min-h-[80vh] w-full items-center justify-center overflow-hidden bg-navy-900 pt-32 pb-20">
      ...
      <div className="relative z-10 mx-auto max-w-[1140px] px-6 text-center">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8 inline-flex items-center gap-2 rounded-full border border-teal-5/30 bg-teal-5/10 px-4 py-2">
          ...
          <span className="text-sm font-semibold text-teal-4">For Employers</span>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto pt-12 border-t border-white/10">
          {[{ v: "6 Tools", l: "Built For Employers" }, { v: "100% Free", l: "No Obligation" }, { v: "Real Data", l: "Updated Live" }, { v: "1st In UK", l: "Industry First" }].map((s, i) => (
            <div key={i} className="text-center"><div className="text-white font-semibold text-2xl mb-1">{s.v}</div><div className="text-[#8B98AB]/70 text-[10px] uppercase tracking-widest font-semibold">{s.l}</div></div>
          ))}
        </motion.div>
      </div>
...
function IntelligenceGrid() {
    const tools = [
        { t: "Staffing Cost Calculator", d: "Calculate your true hourly cost including NI, Pension, and overheads.", href: "/technology/rate-calculator", i: IconCalculator, status: "LIVE" },
        { t: "Real-Time Availability", d: "Live map showing available candidates across our 4 regional hubs.", href: "#", i: IconMap2, status: "LIVE" },
        { t: "Instant Quote Builder", d: "Build a custom staffing quote in 60 seconds. No sales call required.", href: "/technology/quote-builder", i: IconFileInvoice, status: "LIVE" },
        { t: "Industry Dashboard", d: "Average pay rates and worker availability in your specific sector.", href: "#", i: IconChartBar, status: "COMING SOON" },
        { t: "Social Proof Engine", d: "Live feed of our 1,500+ Google Reviews and Trustpilot scores.", href: "#", i: IconStarFilled, status: "LIVE" },
        { t: "Success Story Matcher", d: "Find clients exactly like you and see the results we delivered.", href: "/case-studies", i: IconUserSearch, status: "LIVE" },
        { t: "Compliance Health Check", d: "A 10-point self-audit to see if your current agency is compliant.", href: "#", i: IconShieldCheck, status: "DEVELOPMENT" }
    ];
        ...
        <section className="relative w-full bg-navy-700 py-24 md:py-32 overflow-hidden">
            ...
            <div className="relative z-10 mx-auto max-w-[1140px] px-6 text-center">
                <motion.div className="mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
                    <span className="mb-4 inline-block text-[11px] font-bold uppercase tracking-[0.2em] text-[#00A99D]">CHOOSE YOUR TOOL</span>
                    <h2 className="text-4xl font-semibold text-white tracking-tight">The Intelligence Suite</h2>
                </motion.div>
                <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 text-left" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                    {tools.map((tool, i) => {
                        const CardContent = (
                            ...
                            >
                                <div className="pointer-events-none absolute inset-0 rounded-2xl bg-gradient-to-br from-teal-5/5 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                                 <div className="relative flex items-start justify-between mb-8">
                                     <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${tool.status === 'LIVE' ? 'bg-teal-5/10 text-teal-4 group-hover:bg-[#00A99D] group-hover:text-white' : 'bg-white/5 text-[#8B98AB]/50'} transition-all`}>
                                         <tool.i size={32} />
                                     </div>
                                     <span className={`text-[9px] font-semibold uppercase tracking-widest px-3 py-1 rounded-full ${tool.status === 'LIVE' ? 'bg-teal-5/10 text-teal-4' : 'bg-white/5 text-[#8B98AB]/70'}`}>
                                         {tool.status}
                                     </span>
                                 </div>
                                 <h4 className="relative text-white font-semibold text-xl mb-4 leading-tight">{tool.t}</h4>
                                 <p className="relative text-[#8B98AB] text-sm leading-relaxed mb-10 flex-grow">{tool.d}</p>
                                 <div className="relative pt-8 border-t border-white/5 flex items-center justify-between mt-auto">
                                     <span className={`text-xs font-semibold uppercase tracking-widest ${tool.status === 'LIVE' ? 'text-teal-5 pb-1 border-b border-teal-5/50' : 'text-[#8B98AB]/50'}`}>
                        ...
                        return tool.status === 'LIVE' ? (
                            <Link href={tool.href} key={i} className="block w-full h-full">{CardContent}</Link>
                        ) : (
                            <div key={i} className="block w-full h-full">{CardContent}</div>
                        );
                    })}
                    <div className="hidden lg:flex glass-card rounded-2xl border-dashed items-center justify-center flex-col text-center p-10 opacity-30">
                        ...
                        <p className="text-xs uppercase font-semibold tracking-widest">More Tools In Development</p>
                    </div>
                </motion.div>
            </div>
        </section>
            ...
            <div className="pointer-events-none absolute -right-64 top-1/2 h-[600px] w-[600px] -translate-y-1/2 rounded-full bg-teal-5/10 blur-[130px]" />
             <div className="relative z-10 mx-auto max-w-[1140px] px-6">
                <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
                     <span className="mb-4 inline-block text-[11px] font-bold uppercase tracking-[0.2em] text-[#00A99D]">WHY WE BUILT THIS</span>
                     <h2 className="text-4xl font-semibold text-white tracking-tight">Transparency is a feature.</h2>
                </motion.div>
                ...
                <div className="mt-20 glass-card rounded-2xl p-10 flex flex-col md:flex-row items-center justify-between gap-8 max-w-4xl mx-auto">
                     <div className="text-left">
                         ...
                         <div className="text-white font-semibold text-2xl mb-1">4.8 Stars</div>
                         <div className="text-[#8B98AB]/70 text-xs font-semibold uppercase tracking-widest">Trusted by 1500+ People on Google</div>
                     </div>
