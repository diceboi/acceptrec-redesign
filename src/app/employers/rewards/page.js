  ...
  return (
    <section className="relative flex min-h-[80vh] w-full items-center justify-center overflow-hidden bg-navy-900 pt-32 pb-20">
      ...
      <div className="relative z-10 mx-auto max-w-[1140px] px-6 text-center">
        ...
        >
          The Only Agency That <br />
          <span className="text-teal-5 italic">Rewards You Back.</span>
        </motion.h1>
        <motion.p
        ...
        >
          Most agencies take your money and disappear until next month&apos;s
          invoice. We built a partnership programme that actually rewards you
          for working with us.
        </motion.p>
        <motion.div
        ...
        <motion.div
          ...
          className="mt-16 grid grid-cols-3 gap-8 max-w-2xl mx-auto pt-12 border-t border-white/10"
        >
          {[
            { v: "190+", l: "Partners Enrolled" },
            { v: "2.5M+", l: "Points Earned" },
            { v: "12 wk", l: "Longest Streak" },
          ].map((s, i) => (
            <div key={i}>
              <div className="text-white font-semibold text-2xl">{s.v}</div>
      ...
      <div className="relative z-10 mx-auto max-w-[1140px] px-6">
        ...
        <div className="glass-card max-w-4xl mx-auto rounded-2xl overflow-hidden">
          <div className="grid grid-cols-4 p-6 md:p-8 border-b border-white/10 bg-white/5">
            <div className="col-span-2 text-[10px] md:text-[11px] font-semibold text-[#8B98AB]/70 uppercase tracking-widest">
              Relationship Feature
            </div>
            <div className="text-center text-[10px] md:text-[11px] font-semibold text-[#8B98AB]/70 uppercase tracking-widest px-2">
              Generic Agency
            </div>
            <div className="text-center text-[10px] md:text-[11px] font-semibold text-teal-5 uppercase tracking-widest px-2">
              Accept Recruitment
            </div>
          </div>
          {rows.map((r, i) => (
            <div
              key={i}
              className="grid grid-cols-4 items-center p-6 md:p-8 border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors group"
            >
              <div className="col-span-2 text-white font-semibold text-sm md:text-base pr-4">
                {r.feature}
              </div>
              <div className="flex justify-center">
                {r.trad ? (
                  <IconCircleCheckFilled
                    className="text-[#8B98AB]/30"
                    size={20}
                  />
                ) : (
                  <IconX className="text-red-500/30" size={20} />
                )}
              </div>
              <div className="flex justify-center relative">
                ...
                <IconDiscountCheckFilled
                  className="text-teal-4 relative z-10"
                  size={24}
                />
              </div>
            </div>
...
function EarningMechanics() {
  const ways = [
    {
      t: "Booking Points",
      d: "Earn points for every hour booked. Bonus multipliers for early planning.",
      i: IconBuildingWarehouse,
    },
    {
      t: "Engagement Points",
      d: "Earn points for providing feedback and using the client portal daily.",
      i: IconMessage2,
    },
    {
      t: "Growth Points",
      d: "Earn large point bonuses for site expansions and B2B referrals.",
      i: IconTrendingUp,
    },
  ];
        ...
        >
          {ways.map((w, i) => (
            ...
            >
              ...
              <h4 className="relative text-white font-semibold text-xl mb-4">
                {w.t}
              </h4>
              <p className="relative text-[#8B98AB] text-sm leading-relaxed">
                {w.d}
              </p>
            </motion.div>
          ))}
        ...
        >
          {tiers.map((t, i) => (
            <motion.div
              key={i}
              variants={cardVariants}
              className={`glass-card group relative overflow-hidden rounded-2xl p-10 transition-all duration-300 hover:teal-glow-sm flex flex-col items-center ${t.highlight ? "border-teal-5/20 hover:border-teal-5/40" : "hover:border-teal-5/30"}`}
            >
              <div className="pointer-events-none absolute inset-0 rounded-2xl bg-gradient-to-br from-teal-5/5 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
              <div
                className={`relative w-20 h-20 rounded-2xl bg-white/5 flex items-center justify-center mb-8 ${t.highlight ? "text-teal-4" : "text-[#8B98AB]"} group-hover:bg-white/10 transition-all`}
              >
                <t.icon size={40} />
              </div>
              <h4 className="relative text-white font-semibold text-2xl mb-2">
                {t.name}
              </h4>
              <p className="relative text-[#8B98AB]/70 text-xs font-semibold uppercase tracking-widest mb-10">
                {t.req}
              </p>
              <div className="relative w-full pt-10 border-t border-white/5">
                <div className="text-teal-4 font-semibold text-4xl">
                  {t.multiplier}
                </div>
                <div className="text-[#8B98AB]/70 text-[10px] font-semibold uppercase tracking-widest mt-2">
                  Point Multiplier
                </div>
              </div>
            </motion.div>
      ...
      <div className="mx-auto max-w-[1140px] px-6">
        <div className="bg-teal-5 rounded-2xl p-12 md:p-24 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-12">
          ...
          <div className="relative z-10 max-w-xl text-center md:text-left">
            <h2 className="text-black text-[11px] font-semibold uppercase tracking-widest mb-6 px-4 py-2 bg-black/5 rounded-full inline-block">
              B2B GROWTH
            </h2>
            <h3 className="text-black text-4xl md:text-5xl font-semibold tracking-tight mb-8">
              Refer. Earn. Repeat.
            </h3>
            <p className="text-black/60 text-lg font-semibold leading-relaxed">
              Introduce a new partner business to Accept Recruitment and receive
              a point bonus equivalent to **£250** upon their first successful
              booking.
            </p>
          </div>
