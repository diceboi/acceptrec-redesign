    ...
    <section className="relative flex min-h-[80vh] w-full flex-col items-center justify-center overflow-hidden bg-navy-900 pt-32 pb-16">
      ...
      <div className="relative z-10 mx-auto max-w-[1140px] px-6 text-center">
        ...
        >
          ...
          <span className="text-[11px] font-semibold tracking-[0.2em] uppercase text-teal-5">Intelligence Tools</span>
        </motion.div>
        ...
        >
          The Numbers Every <br /><span className="text-teal-5">Client Should Know</span>
        </motion.h1>
...
function InteractiveCalculator() {
  ...
  useEffect(() => {
    ...
    setCosts({ holidayPay, employerNI, pension, niOnHoliday, sspProvision, appLevy, totalStat });
  }, [baseRate]);
      ...
      <div className="relative z-10 mx-auto max-w-[1140px] px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="glass-card rounded-2xl p-10 relative overflow-hidden hover:border-teal-5/30 transition-all">
                ...
                <div className="mb-12">
                   <div className="flex items-end justify-between mb-6">
                      <div className="text-[#8B98AB] text-xs font-semibold uppercase">Base Hourly Pay</div>
                      <div className="text-white text-5xl font-mono font-semibold tracking-tighter">{formatCurrency(baseRate)}</div>
                   </div>
                   <input 
                      type="range" 
                      min="11.44" 
                ...
                <div className="p-8 rounded-2xl bg-teal-5/5 border border-teal-5/10">
                    <div className="text-teal-5/60 text-xs font-semibold uppercase tracking-widest mb-2">Your True Cost (Before Margin)</div>
             ...
             <div className="glass-card rounded-2xl p-8">
                <div className="flex gap-4 items-start">
                    ...
                    <div>
                        <h4 className="text-white font-semibold mb-2">Statutory Compliance</h4>
          ...
          <div className="glass-card rounded-2xl p-10">
                <h3 className="text-white text-sm font-semibold uppercase tracking-widest mb-10">Statutory Breakdown (Hourly)</h3>
                ...
                <div className="space-y-1">
                    {[
                        ...
                        <div key={i} className={`flex items-center justify-between p-5 rounded-2xl transition-colors hover:bg-white/5 ${row.bold ? 'bg-white/5 border border-white/5 mb-4' : ''}`}>
                            <div>
                                <div className={`text-sm font-semibold ${row.bold ? 'text-white' : 'text-white/80'}`}>{row.label}</div>
                                <div className="text-[10px] text-[#8B98AB]/70 font-semibold uppercase tracking-widest mt-0.5">{row.sub}</div>
                            </div>
                            <div className={`font-mono text-lg font-semibold ${row.bold ? 'text-white' : 'text-teal-4/80'}`}>{formatCurrency(row.value)}</div>
                        </div>
                    ))}
                </div>

                <div className="mt-10 pt-10 border-t border-white/10 flex items-center justify-between">
                    <div className="text-white font-semibold text-lg">Total Cost Foundation</div>
                    <div className="text-white text-2xl font-mono font-semibold">{formatCurrency(costs.totalStat)}</div>
                </div>
          </div>
        ...
        <section className="py-24 bg-[#0d111a] relative overflow-hidden">
            ...
            <div className="relative z-10 mx-auto max-w-[1140px] px-6">
                <div className="max-w-3xl mb-16">
                    <span className="mb-4 inline-block text-[11px] font-bold uppercase tracking-[0.2em] text-[#00A99D]">THE VARIABLE</span>
                    <h2 className="text-4xl font-semibold text-white mb-8 tracking-tight">Why Our Margin Isn&apos;t Fixed</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    {MARGIN_TIERS.map((tier, i) => (
                        <div key={i} className="glass-card rounded-2xl p-10 hover:border-teal-5/30 transition-all group flex flex-col h-full hover:teal-glow-sm">
                            ...
                            <div className="relative mb-8">
                                <h4 className="text-white text-xl font-semibold mb-1">{tier.name}</h4>
                                <div className="text-teal-5 font-mono font-semibold text-sm tracking-tight">{tier.range}/hr</div>
                                <div className="text-[#8B98AB]/70 text-[10px] font-semibold uppercase tracking-widest mt-1">{tier.desc}</div>
                            </div>
    ...
    <section className="py-24 bg-navy-900">
      <div className="mx-auto max-w-[1140px] px-6">
        <div className="glass-card rounded-2xl p-12 md:p-20 relative overflow-hidden text-center">
            ...
            <IconInfoCircle className="relative mx-auto text-teal-5 mb-8" size={48} strokeWidth={1.5} />
            <h3 className="relative text-white text-3xl font-semibold mb-8 tracking-tight">Legal Requirements Are Unavoidable</h3>
            ...
            <div className="relative mt-12 flex flex-wrap justify-center gap-12">
                {[
                    ...
                    <div key={i}>
                        <div className="text-white text-2xl font-semibold mb-1">{item.value}</div>
                        <div className="text-teal-5 text-[10px] font-semibold uppercase tracking-widest">{item.label}</div>
                    </div>
                ))}
            </div>
        </div>
