    ...
    <section className="relative flex min-h-[80vh] w-full flex-col items-center justify-center overflow-hidden bg-navy-900 pt-32 pb-16">
      ...
      <div className="relative z-10 mx-auto max-w-[1140px] px-6 text-center">
        ...
        >
          ...
          <span className="text-[11px] font-semibold tracking-[0.2em] uppercase text-teal-5">Instant Estimate</span>
        </motion.div>
        ...
        >
          Instant Staffing <br /><span className="text-teal-5">Quote Builder</span>
        </motion.h1>
...
function QuoteBuilderTool() {
  ...
  useEffect(() => {
    ...
    setResults({ hourlyCharge, weeklyTotal, monthlyTotal, complexity });
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-12">
            ...
            <div className="xl:col-span-7 space-y-10">
                <div className="glass-card rounded-2xl p-10">
                    <h3 className="text-white text-sm font-semibold uppercase tracking-widest mb-12 flex items-center gap-2">
                    ...
                    <div className="space-y-12">
                        ...
                        <div>
                            <div className="flex justify-between items-end mb-6">
                                <div>
                                    <div className="text-white font-semibold mb-1">Number of Workers</div>
                                    <div className="text-[#8B98AB]/70 text-[10px] font-semibold uppercase tracking-widest">How many temps do you need daily?</div>
                                </div>
                                <div className="text-teal-5 text-4xl font-mono font-semibold tracking-tighter">{workers}</div>
                            </div>
                            <input 
                        ...
                        {/* Role Picker */}
                        <div>
                            <div className="text-white font-semibold mb-6">Role Type</div>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {ROLES.map(r => (
                                    <button 
                                        key={r.id}
                                        onClick={() => setRoleId(r.id)}
                                        className={`p-4 rounded-2xl border transition-all text-left group ${roleId === r.id ? 'bg-teal-5 border-teal-5 text-white' : 'bg-white/5 border-white/5 text-[#8B98AB] hover:border-white/20'}`}
                                    >
                        ...
                        {/* Shift Picker */}
                        <div>
                            <div className="text-white font-semibold mb-6">Shift Pattern</div>
                            <div className="grid grid-cols-3 gap-3">
                                {SHIFTS.map(s => (
                                    <button 
                                        key={s.id}
                                        onClick={() => setShiftId(s.id)}
                                        className={`py-3 rounded-xl border transition-all text-[11px] font-semibold uppercase tracking-widest ${shiftId === s.id ? 'bg-teal-5 border-teal-5 text-white' : 'bg-white/5 border-white/5 text-[#8B98AB] hover:border-white/20'}`}
                                    >
                                        {s.label}
                                    </button>
                                ))}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-4">
                             <div>
                                <div className="text-white font-semibold mb-6">Engagement Duration</div>
                                <div className="space-y-3">
                                    {ENGAGEMENTS.map(e => (
                                        <button 
                                            key={e.id}
                                            onClick={() => setEngagementId(e.id)}
                                            className={`w-full p-4 rounded-2xl border transition-all text-left flex items-center justify-between group ${engagementId === e.id ? 'bg-teal-5 border-teal-5 text-white' : 'bg-white/5 border-white/5 text-[#8B98AB] hover:border-white/20'}`}
                                        >
                                            <div className="text-xs font-semibold leading-tight">{e.label}</div>
                                            <div className={`text-[9px] font-semibold uppercase tracking-widest ${engagementId === e.id ? 'text-white/70' : 'text-[#8B98AB]/70'}`}>{e.sub}</div>
                                        </button>
                                    ))}
                                </div>
                             </div>
                             <div>
                                <div className="flex justify-between items-end mb-6">
                                    <div>
                                        <div className="text-white font-semibold mb-1">Hours / Week</div>
                                        <div className="text-[#8B98AB]/70 text-[10px] font-semibold uppercase tracking-widest">Average per worker</div>
                                    </div>
                                    <div className="text-teal-5 text-2xl font-mono font-semibold">{hours}</div>
                                </div>
                                <input 
            ...
            <div className="xl:col-span-5">
                <div className="sticky top-32 space-y-8">
                    <div className="glass-card rounded-2xl border-white/10 p-10 relative overflow-hidden">
                        <div className="absolute -top-24 -right-24 w-64 h-64 bg-teal-5/10 blur-[80px] rounded-full pointer-events-none" />
                        
                        <div className="relative flex items-center justify-between mb-10">
                            <h3 className="text-white text-sm font-semibold uppercase tracking-widest">Estimated Totals</h3>
                        ...
                        <div className="relative space-y-8 mb-12">
                            <div>
                                <div className="text-[#8B98AB] text-[11px] font-semibold uppercase tracking-widest mb-2">Weekly Investment</div>
                                <motion.div 
                                    key={results.weeklyTotal}
                                    initial={{ opacity: 0.5 }}
                                    animate={{ opacity: 1 }}
                                    className="text-white text-5xl md:text-6xl font-mono font-semibold tracking-tighter"
                                >
                                    {formatCurrency(results.weeklyTotal)}
                                </motion.div>
                            </div>
                            <div className="pt-8 border-t border-white/5">
                                <div className="text-[#8B98AB] text-[11px] font-semibold uppercase tracking-widest mb-2">Monthly Estimate (x4.33)</div>
                                <motion.div 
                                    key={results.monthlyTotal}
                                    initial={{ opacity: 0.5 }}
                                    animate={{ opacity: 1 }}
                                    className="text-white text-3xl md:text-4xl font-mono font-semibold tracking-tighter opacity-60"
                                >
                        ...
                        <div className="relative space-y-4 mb-10">
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-[#8B98AB] font-medium">Hourly Rate / Worker</span>
                                <span className="text-white font-mono font-semibold text-lg">{formatCurrency(results.hourlyCharge)}</span>
                            </div>
                            <p className="text-[#8B98AB]/70 text-[10px] leading-relaxed italic">
                    ...
                    <div className="glass-card rounded-2xl p-8">
                        <div className="flex gap-4 items-start">
                            ...
                            <div>
                                <h4 className="text-white font-semibold mb-2 text-sm italic">Why Does Pricing Vary?</h4>
                                <ul className="space-y-3">
                                    {[
                                        ...
                                        <li key={i} className="flex gap-2 text-[11px] text-[#8B98AB] font-medium leading-tight">
        ...
        <section className="py-24 bg-[#0d111a]">
            <div className="mx-auto max-w-[1140px] px-6">
                <div className="max-w-3xl">
                    <h3 className="text-white text-3xl font-semibold mb-8 tracking-tight">This Is An Estimate Only</h3>
                    <p className="text-white/60 text-xl leading-relaxed mb-8">
                        Actual pricing depends on your specific requirements, location, shift patterns, and volume. This calculator gives you a ballpark figure, but <span className="text-white font-semibold italic underline decoration-teal-5 underline-offset-4">we need to visit your site</span> to provide an accurate quote.
                    </p>
