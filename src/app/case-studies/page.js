    ...
    <section className="relative flex min-h-[80vh] w-full items-center justify-center overflow-hidden bg-navy-900 pt-32 pb-20">
      ...
      <div className="relative z-10 mx-auto max-w-[1140px] px-6 text-center">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8 inline-flex items-center gap-2 rounded-full border border-teal-5/30 bg-teal-5/10 px-4 py-2">
          ...
          <span className="text-sm font-semibold text-teal-4">Real Results</span>
        </motion.div>
        ...
        <div className="relative grid grid-cols-3 gap-4 py-6 border-y border-white/5 mb-8">
          {study.stats.map((s, i) => (
            <div key={i} className="text-center">
              <div className="text-white font-semibold text-lg mb-0.5 whitespace-nowrap">{s.value}</div>
              <div className="text-[#8B98AB] text-[10px] uppercase font-bold tracking-widest">{s.label}</div>
            </div>
          ))}
        </div>
